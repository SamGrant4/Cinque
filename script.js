...
// function makes it so that we can click things
function startInteraction() {
    document.addEventListener("click", handleMouseClick)
    document.addEventListener("keydown", handleKeyPress)
}

// function makes it so that things are no longer clickable
...
function handleMouseClick(e) {
    // if one of the letter keys are clicked...
    if (e.target.matches("[data-key]")) {
        pressKey(e.target.dataset.key)
        return
    }

    // if enter is clicked submit guess
    if (e.target.matches("[data-enter]")) {
        submitGuess()
        return
    }

    // if delete is clicked delete last entered
...
// function takes a key press and shows that letter in grid
function pressKey(key) {
    // gets number of current active tiles
    const activeTiles = getActiveTiles()

    // if row is full return 
    if (activeTiles.length >= WORD_LENGTH) return

    // finds the first tile that doesn't have a letter
    const nextTile = guessGrid.querySelector(":not([data-letter])")

    // sets the next tile to the key pressed
    nextTile.dataset.letter = key.toLowerCase()
    nextTile.textContent = key
    nextTile.dataset.state = "active"
}

// function deletes last letter from grid
function deleteKey() {
    // gets number of current active tiles
    const activeTiles = getActiveTiles()
...
function submitGuess() {
    ...
    // takes each tile and makes it into a guess (a string of 5 letters)
    const guess = activeTiles.reduce((word, tile) => {
        return word + tile.dataset.letter
    }, "")

    // if guess is not in dictionary then alert
...
/* WIN/LOSE LOGIC & ANIMATION */

// function checks if win or loss
function checkWinLose(guess, tiles) {
    // if guess is correct
    if (guess === targetWord) {
        showAlert("You Win", 5000) // show win alert
        danceTiles(tiles) // make tiles dance
        stopInteraction() // stops interaction
        return
    }
...
// function that makes tiles dance 
